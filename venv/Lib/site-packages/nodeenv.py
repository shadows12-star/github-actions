...
class Config:
    ...
    def _dump(cls):
        ...
        print("    [nodeenv]")
        print("    " + "\n    ".join(
            f"{k} = {v}" for k, v in sorted(iteritems(vars(cls)))
...
def parse_args(check=True):
    ...
    if args.config_file is None:
        ...
        if not os.path.exists(args.config_file):
            parser.error("Config file '{}' doesn't exist!".format(
                args.config_file))
...
def get_root_url(version_str):
    if parse_version(version_str) > (0, 5):
        return f'{src_base_url}/v{version_str}/'
    else:
        return src_base_url
...
def get_node_bin_url(version):
    ...
    filename = f'node-v{version}{postfix}'
...
def urlopen(url):
    ...
    headers = {'User-Agent': f'nodeenv/{nodeenv_version} ({home_url})'}
...
def install_node_wrapped(env_dir, src_dir, args):
    ...
    logger.info(f' * Install {src_type} node ({args.node}) ',
